...
public class OssUtil {
    ...
    public boolean upload(String bucket, String key, File file) {
        checkBucket(bucket);
        return s3Client.putObject(builder -> builder.bucket(bucket).key(key).contentType(HttpUtil.getMimeType(file.getPath())), RequestBody.fromFile(file)).sdkHttpResponse().isSuccessful();
    }


    public boolean upload(String bucket, String key, ByteBuffer byteBuffer, String contentType) {
        checkBucket(bucket);
        return s3Client.putObject(builder -> builder.bucket(bucket).key(key).contentType(contentType), RequestBody.fromByteBuffer(byteBuffer)).sdkHttpResponse().isSuccessful();
    ...
    public boolean deleteFile(String bucket, String... keys) {
        ...
        return s3Client.deleteObjects(builder -> builder.bucket(bucket).delete(deleteBuilder -> deleteBuilder.objects(willDelete))).hasDeleted();
    }
    ...
    public String getUrl(String bucket, String fileKey) {
        return s3Presigner.presignGetObject(GetObjectPresignRequest.builder().signatureDuration(Duration.ofMinutes(60)).getObjectRequest(GetObjectRequest.builder().bucket(bucket).key(fileKey).build()).build()).url().toString();
    }
}
